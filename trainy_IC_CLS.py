...
class ClipCocoDataset(Dataset):
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, ...]:
        ...
        return tokens, mask, prefix, self.cls[item]
    ...
    def __init__(self, data_path: str, prefix_length: int, gpt2_type: str = "gpt2",
        ...
        self.cls = [caption['cls'] for caption in captions_raw]
...
def train(model: ClipCaptionModel, train_dataset: ClipCocoDataset,
    ...
    for epoch in range(epochs):
        ...
        train_loss = 0
        for idx, (tokens, mask, prefix, clf) in enumerate(train_dataloader):
            model.train()
            model.zero_grad()
            tokens, mask, prefix, clf = tokens.to(device), mask.to(device), prefix.to(device,
                                                                                      dtype=torch.float32), clf.to(device)

            # Pass the input tensor through the Discriminator
            disc_output = discriminator(prefix)
            if clf == 1:
                outputs = model(tokens, prefix, mask)
                logits = outputs.logits[:, train_dataset.prefix_length - 1: -1]
                loss = nnf.cross_entropy(logits.reshape(-1, logits.shape[-1]), tokens.flatten(),
                                         ignore_index=0) + criterion(disc_output, clf.squeeze())
            else:
                loss = criterion(disc_output, clf.squeeze())
...
def main():
    myconfig = {
        'epochs': 6,
        'batch_size': 1,
        'train_data': '/home/chris/PycharmProjects/CLIP_prefix_caption/checkpoints/cls_1k_combined_gen_clip_feat_train_ic.pkl',
    ...
    train(model, train_dataset, train_dataset, myconfig, output_dir=myconfig.get('out_dir'),

...
class Predictor():
    ...
    def predict(self, image_path, use_beam_search):
        ...
        model = self.model
        gather_images = []
        if isinstance(temp_img_list,list):
            for t in temp_img_list:
                image = io.imread(t)
        ...
        with torch.no_grad():
            prefix = self.clip_model.encode_image(images).to(
                self.device, dtype=torch.float32
            )

            prefix_embed = model.clip_project(prefix)
        if use_beam_search:
            output = generate_beam(model, self.tokenizer, embed=prefix_embed)
        else:
            output =  generate2(model, self.tokenizer, embed=prefix_embed)

        for image,output in zip(temp_img_list,output):
...
if __name__ == '__main__':
    ...
    temp_img_list = ['Images/CONCEPTUAL_04.jpg',
                     'Images/CONCEPTUAL_05.jpg',
                     'Images/CONCEPTUAL_06.jpg']

    output = mypredictor.predict(image_path=temp_img_list, use_beam_search=False)
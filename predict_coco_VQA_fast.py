...
if __name__ == '__main__':
    ...
    # todo
    temp_path  = './coco_MTL_Diffusion/coco_MTL_Diffusion-009.pt'
    mypredictor = Predictor(weights_path=temp_path)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    # todo
    val_dataset = ClipCocoDataset(
        '/scratch/chris.morfopoulos/data/coco/clip_feat_ViT-B_32_test_vqa.pkl',
        10, normalize_prefix=False)
    qs = val_dataset.questions
    im = val_dataset.prefixes

    temp_path = '/scratch/chris.morfopoulos/data/coco/annotations/vqa_test2015_questions.json'
    ...
    with open("./dict_coco_MTL_Diffusion_vqa.json", "w") as outfile:
